...
if (!function_exists('organic_beauty_essgrids_theme_setup')) {
	...
	function organic_beauty_essgrids_theme_setup() {
		...
		if (organic_beauty_exists_essgrids()) {
			if (is_admin()) {
				add_filter( 'organic_beauty_filter_importer_options',			'organic_beauty_essgrids_importer_set_options', 10, 1 );
				add_action( 'organic_beauty_action_importer_params',			'organic_beauty_essgrids_importer_show_params', 10, 1 );
				add_action( 'organic_beauty_action_importer_clear_tables',	'organic_beauty_essgrids_importer_clear_tables', 10, 2 );
				add_action( 'organic_beauty_action_importer_import',			'organic_beauty_essgrids_importer_import', 10, 2 );
				add_filter( 'organic_beauty_filter_importer_import_row',		'organic_beauty_essgrids_importer_check_row', 9, 4);
				add_action( 'organic_beauty_action_importer_import_fields',	'organic_beauty_essgrids_importer_import_fields', 10, 1 );
			}
		}
		if (is_admin()) {
...
if ( !function_exists( 'organic_beauty_essgrids_importer_required_plugins' ) ) {
	//add_filter( 'organic_beauty_filter_importer_required_plugins',	'organic_beauty_essgrids_importer_required_plugins', 10, 2 );
	function organic_beauty_essgrids_importer_required_plugins($not_installed='', $list='') {
		if (organic_beauty_strpos($list, 'essgrids')!==false && !organic_beauty_exists_essgrids() )
			$not_installed .= '<br>'.esc_html__('Essential Grids', 'organic-beauty');
		return $not_installed;
	}
}

// Set plugin's specific importer options
if ( !function_exists( 'organic_beauty_essgrids_importer_set_options' ) ) {
	//add_filter( 'organic_beauty_filter_importer_options',	'organic_beauty_essgrids_importer_set_options', 10, 1 );
	function organic_beauty_essgrids_importer_set_options($options=array()) {
		if ( in_array('essgrids', (array)organic_beauty_storage_get('required_plugins')) && organic_beauty_exists_essgrids() ) {
...
if ( !function_exists( 'organic_beauty_essgrids_importer_show_params' ) ) {
	//add_action( 'organic_beauty_action_importer_params',	'organic_beauty_essgrids_importer_show_params', 10, 1 );
	function organic_beauty_essgrids_importer_show_params($importer) {
		$importer->show_importer_params(array(
			'slug' => 'essgrids',
			'title' => esc_html__('Import Essential Grid', 'organic-beauty'),
			'part' => 1
			));
	}
}
...
// Clear tables
if ( !function_exists( 'organic_beauty_essgrids_importer_clear_tables' ) ) {
	//add_action( 'organic_beauty_action_importer_clear_tables',	'organic_beauty_essgrids_importer_clear_tables', 10, 2 );
	function organic_beauty_essgrids_importer_clear_tables($importer, $clear_tables) {
		if (organic_beauty_exists_essgrids() && in_array('essgrids', (array)organic_beauty_storage_get('required_plugins'))) {
			if (strpos($clear_tables, 'essgrids')!==false) {
				if ($importer->options['debug']) dfl(__('Clear Essential Grid tables', 'organic-beauty'));
				global $wpdb;
				$res = $wpdb->query("TRUNCATE TABLE " . esc_sql($wpdb->prefix) . Essential_Grid::TABLE_GRID);
				if ( is_wp_error( $res ) ) dfl( sprintf(__( 'Failed truncate table "%s". Error message: %s', 'organic-beauty' ), Essential_Grid::TABLE_GRID, $res->get_error_message()) );
			}
		}
	}
}
...
// Import posts
if ( !function_exists( 'organic_beauty_essgrids_importer_import' ) ) {
	//add_action( 'organic_beauty_action_importer_import',	'organic_beauty_essgrids_importer_import', 10, 2 );
	function organic_beauty_essgrids_importer_import($importer, $action) {
			...
			if ( ($txt = $importer->get_file($importer->options['files'][$importer->options['demo_type']]['file_with_essgrids'])) != '') {
				...
				try {
					...
					if ($importer->options['debug']) 
						dfl( __('Essential Grid import complete', 'organic-beauty') );
							
				} catch (Exception $d) {
					$msg = sprintf(esc_html__('Essential Grid import error: %s', 'organic-beauty'), $d->getMessage());
...
if ( !function_exists( 'organic_beauty_essgrids_importer_check_row' ) ) {
	//add_filter('organic_beauty_filter_importer_import_row', 'organic_beauty_essgrids_importer_check_row', 9, 4);
	function organic_beauty_essgrids_importer_check_row($flag, $table, $row, $list) {
...
if ( !function_exists( 'organic_beauty_essgrids_importer_import_fields' ) ) {
	//add_action( 'organic_beauty_action_importer_import_fields',	'organic_beauty_essgrids_importer_import_fields', 10, 1 );
	function organic_beauty_essgrids_importer_import_fields($importer) {
		$importer->show_importer_fields(array(
			'slug' => 'essgrids',
			'title' => esc_html__('Essential Grid', 'organic-beauty')
			));
	}
}
?>