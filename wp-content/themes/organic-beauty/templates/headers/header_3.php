...
if ( !function_exists( 'organic_beauty_template_header_3_theme_setup' ) ) {
	add_action( 'organic_beauty_action_before_init_theme', 'organic_beauty_template_header_3_theme_setup', 1 );
	function organic_beauty_template_header_3_theme_setup() {
		organic_beauty_add_template(array(
			'layout' => 'header_3',
			'mode'   => 'header',
			'title'  => esc_html__('Header 3', 'organic-beauty'),
			'icon'   => organic_beauty_get_file_url('templates/headers/images/3.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'organic_beauty_template_header_3_output' ) ) {
	function organic_beauty_template_header_3_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_3 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_3 top_panel_position_<?php echo esc_attr(organic_beauty_get_custom_option('top_panel_position')); ?>">
