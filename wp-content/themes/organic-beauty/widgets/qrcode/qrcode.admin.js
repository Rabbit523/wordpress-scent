...
jQuery(document).ready(function(){
	organic_beauty_qrcode_init();
	organic_beauty_color_picker();
});

function organic_beauty_qrcode_init() {
	jQuery('#widgets-right .widget_qrcode:not(.inited)').each(function() {
		...
		widget.find('input.iColorPicker:not(.colored)').each(function() {
			...
			if (obj.attr('id').indexOf('__i__') < 0) {
				...
				organic_beauty_set_color_picker(obj.attr('id'));
			}
		});
		widget.find('div.qrcode_tabs').tabs();
		widget.on('click', '.fld_button_draw', function() {
			organic_beauty_qrcode_update(widget);
		});
...
function organic_beauty_qrcode_update(widget) {
	organic_beauty_qrcode_show(widget, {
...
function organic_beauty_qrcode_show(widget, vc, opt) {
	if (opt.show_what==1) {
...
if (!window.organic_beauty_set_color_picker) {
	function organic_beauty_set_color_picker(id_picker) {
