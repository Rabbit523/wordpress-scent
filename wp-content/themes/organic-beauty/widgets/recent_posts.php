...
// Theme init
if (!function_exists('organic_beauty_widget_recent_posts_theme_setup')) {
	add_action( 'organic_beauty_action_before_init_theme', 'organic_beauty_widget_recent_posts_theme_setup', 1 );
	function organic_beauty_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('organic_beauty_exists_visual_composer') && organic_beauty_exists_visual_composer())
			add_action('organic_beauty_action_shortcodes_list_vc','organic_beauty_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('organic_beauty_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'organic_beauty_widget_recent_posts_load' );
	function organic_beauty_widget_recent_posts_load() {
		register_widget('organic_beauty_widget_recent_posts');
	}
}

// Widget Class
class organic_beauty_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'organic-beauty'));
		parent::__construct( 'organic_beauty_widget_recent_posts', esc_html__('Organic Beauty - Recent Posts', 'organic-beauty'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'organic-beauty')), organic_beauty_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'organic_beauty_sc_widget_recent_posts' ) ) {
	function organic_beauty_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'organic_beauty_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, organic_beauty_prepare_widgets_args(organic_beauty_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('organic_beauty_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	organic_beauty_require_shortcode("trx_widget_recent_posts", "organic_beauty_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('organic_beauty_widget_recent_posts_reg_shortcodes_vc')) {
	function organic_beauty_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'organic-beauty'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'organic-beauty') ),
