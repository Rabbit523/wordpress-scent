...
function organic_beauty_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = ORGANIC_BEAUTY_STORAGE['co_add_params']!=undefined ? ORGANIC_BEAUTY_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#organic_beauty-customizer-styles-'+option).length > 0 ? jQuery('#organic_beauty-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="organic_beauty-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
