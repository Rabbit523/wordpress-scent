...
// Theme init
if (!function_exists('organic_beauty_importer_theme_setup')) {
	add_action( 'organic_beauty_action_after_init_theme', 'organic_beauty_importer_theme_setup' );		// Fire this action after load theme options
	function organic_beauty_importer_theme_setup() {
		if (is_admin() && current_user_can('import') && organic_beauty_get_theme_option('admin_dummy_data')=='yes') {
			new organic_beauty_dummy_data_importer();
		}
	}
}

class organic_beauty_dummy_data_importer {
	...
	function __construct() {
		...
		// AJAX handler
		add_action('wp_ajax_organic_beauty_importer_start_import',		array($this, 'importer'));
		add_action('wp_ajax_nopriv_organic_beauty_importer_start_import',	array($this, 'importer'));
		// AJAX handler of the get_list_pages actions
		add_action('wp_ajax_organic_beauty_importer_get_list_pages',		array($this, 'get_list_pages_callback'));
		add_action('wp_ajax_nopriv_organic_beauty_importer_get_list_pages',array($this, 'get_list_pages_callback'));
		// Check if row will be imported in the set='part'
		add_filter('organic_beauty_filter_importer_import_row',			array($this, 'import_check_row'), 9, 4);
	}
	...
	function prepare_vars() {
		...
		$this->options['debug'] = organic_beauty_param_is_on(organic_beauty_get_theme_option('debug_mode'));
	    $this->options = apply_filters('organic_beauty_filter_importer_options', $this->options);
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			organic_beauty_admin_add_menu_item('theme', array(
				'page_title' => esc_html__('Install Dummy Data', 'organic-beauty'),
				'menu_title' => esc_html__('Install Dummy Data', 'organic-beauty'),
	...
	function admin_scripts() {
		wp_enqueue_style('p-xel-importer-style',  organic_beauty_get_file_url('core/core.importer/core.importer.css'), array(), null );
		wp_enqueue_script('p-xel-importer-script', organic_beauty_get_file_url('core/core.importer/core.importer.js'), array('jquery'), null, true );
	}
	...
	function build_page() {
		...
		if ( isset($_POST['exporter_action']) ) {
			if ( !wp_verify_nonce( organic_beauty_get_value_gp('nonce'), admin_url() ) )
				$this->error = esc_html__('Incorrect WP-nonce data! Operation canceled!', 'organic-beauty');
		...
		<div class="trx_importer">
			...
			<div class="trx_importer_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Organic Beauty Importer', 'organic-beauty'); ?></h2>

				<form id="trx_importer_form">

					<?php if (count($this->options['files']) > 1) { ?>
						<p><b><?php esc_html_e('Select the demo-data set to be imported:', 'organic-beauty'); ?></b></p>
					...
					<div class="trx_importer_demo_set">
						<label><input type="radio"<?php if ($this->options['demo_set']=='part') echo ' checked="checked"'; ?> value="part" name="demo_set" /><?php esc_html_e('Only pages, forms and sliders', 'organic-beauty'); ?></label>
						<div class="trx_importer_description trx_importer_description_part<?php if ($this->options['demo_set']!='part') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>Only pages, forms layouts and sliders will be imported in this case.</b>', 'organic-beauty')); ?></li>
								<li><?php echo wp_kses_data(__('The imported data will be added to the existing content.', 'organic-beauty')); ?></li>
								<li><?php echo wp_kses_data(__('All the images are replaced with placeholders.', 'organic-beauty')); ?></li>
								<li><?php echo wp_kses_data(__('The new pages provide sample markup and shall not be included in the menu! You have to do it yourself.', 'organic-beauty')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'organic-beauty')); ?></li>
							</ol>
						</div>
						<label><input type="radio"<?php if ($this->options['demo_set']=='full') echo ' checked="checked"'; ?> value="full" name="demo_set" /><?php esc_html_e('Whole demo-site content', 'organic-beauty'); ?></label>
						<div class="trx_importer_description trx_importer_description_full<?php if ($this->options['demo_set']!='full') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>It is recommended for new installations of WordPress</b> (without posts, pages and any other data)!', 'organic-beauty')); ?></li>
								<li><?php echo wp_kses_data(__('As a result, you will get a new set of posts, pages, menu items - <b>a complete copy of our demo site</b>.', 'organic-beauty')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'organic-beauty')); ?></li>
							</ol>
						</div>
					</div>

					<p><b><?php esc_html_e('Select the elements to be imported:', 'organic-beauty'); ?></b></p>

					<?php
					$this->show_importer_params(array(
						'slug' => 'posts',
						'title' => esc_html__('Import posts, pages, taxonomies, etc.', 'organic-beauty'),
						'part' => 1,
						'checked' => true,
					...
					$this->show_importer_params(array(
						'slug' => 'uploads',
						'title' => esc_html__('Import media', 'organic-beauty'),
						'part' => 0,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					$this->show_importer_params(array(
						'slug' => 'tm',
						'title' => esc_html__('Import Theme mods', 'organic-beauty'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'to',
						'title' => esc_html__('Import Theme Options', 'organic-beauty'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'tpl',
						'title' => esc_html__('Import Templates Options', 'organic-beauty'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'widgets',
						'title' => esc_html__('Import Widgets', 'organic-beauty'),
						'part' => 1,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					do_action('organic_beauty_action_importer_params', $this);
					?>
	
					<div class="trx_buttons">
						<input type="button" value="<?php esc_attr_e('Start import', 'organic-beauty'); ?>">
					</div>
				</form>
				
				<div id="trx_importer_progress" class="notice notice-info style_<?php echo esc_attr($this->options['demo_style']); ?>">
					<h4 class="trx_importer_progress_title"><?php esc_html_e('Import demo data', 'organic-beauty'); ?></h4>
					<table border="0" cellpadding="4">
					<?php
					$fields = array(
						'posts'		=> esc_html__('Posts', 'organic-beauty'),
						'uploads'	=> esc_html__('Media', 'organic-beauty'),
						'tm'		=> esc_html__('Theme Mods', 'organic-beauty'),
						'to'		=> esc_html__('Theme Options', 'organic-beauty'),
						'tpl'		=> esc_html__('Template Options', 'organic-beauty'),
						'widgets'	=> esc_html__('Widgets', 'organic-beauty'),
					);
					foreach ($fields as $slug=>$title) {
						$this->show_importer_fields(array('slug' => $slug, 'title' => $title));
					}
					do_action('organic_beauty_action_importer_import_fields', $this);
					?>
					</table>
					<h4 class="trx_importer_progress_complete"><?php esc_html_e('Congratulations! Data import complete!', 'organic-beauty'); ?> <a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('View site', 'organic-beauty'); ?></a></h4>
				</div>
			...
			<div class="trx_exporter_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Organic Beauty Exporter', 'organic-beauty'); ?></h2>

				<?php 
				if ($this->error) {
					?><div class="trx_exporter_error notice notice-error"><?php organic_beauty_show_layout($this->error); ?></div><?php
				}
				?>
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					if ( isset($_POST['exporter_action']) ) { 
						...
						$fields = array(
							'users'			=> esc_html__('Users', 'organic-beauty'),
							'posts'			=> esc_html__('Posts', 'organic-beauty'),
							'theme_mods'	=> esc_html__('Theme Mods', 'organic-beauty'),
							'theme_options'	=> esc_html__('Theme Options', 'organic-beauty'),
							'templates_options'	=> esc_html__('Templates Options', 'organic-beauty'),
							'widgets'		=> esc_html__('Widgets', 'organic-beauty'),
						);
						foreach ($fields as $slug=>$title) {
							$this->show_exporter_fields(array('slug' => $slug, 'title' => $title));
						}
						do_action('organic_beauty_action_importer_export_fields', $this);
						?></table><?php
						...
						if (false && count($this->options['files']) > 1) {
							...
							?><p><b><?php esc_html_e('Select the demo type to be exported', 'organic-beauty'); ?></b></p><?php
						...
						?>
						<div class="trx_buttons">
							<input type="submit" value="<?php esc_attr_e('Export Demo Data', 'organic-beauty'); ?>">
						</div>
						<?php
					}
					?>
	...
	function show_importer_params($args=array()) {
		...
		<label<?php if (!empty($args['class'])) echo ' class="'.esc_attr($args['class']).'"'; ?>>
			<input type="checkbox"
					...
					echo (isset($args['checked']) && $args['checked']) || (in_array($args['slug'], (array)organic_beauty_storage_get('required_plugins')) && $this->options['plugins_initial_state'])
								? ' checked="checked"' 
								: '';
					?>
					value="1"
			...
			<?php organic_beauty_show_layout($args['title']); ?>
		</label>
		<?php
	}
	...
	function show_importer_fields($args=array()) {
		...
		<tr class="import_<?php echo esc_attr($args['slug']); ?>">
			<td class="import_progress_item"><?php organic_beauty_show_layout($args['title']); ?></td>
	...
	function show_exporter_fields($args=array()) {
		...
		?>
		<tr>
			<th align="left"><?php organic_beauty_show_layout($args['title']); ?></th>
			<td><a download="<?php echo esc_attr($args['slug']); ?>.txt" href="<?php echo esc_url(organic_beauty_get_file_url('core/core.importer/export/'.trim($args['slug']).'.txt')); ?>"><?php esc_html_e('Download', 'organic-beauty'); ?></a></td>
		</tr>
		<?php
	}
	...
	function check_required_plugins($list='') {
		$not_installed = '';
		if (in_array('trx_utils', (array)organic_beauty_storage_get('required_plugins')) && !defined('TRX_UTILS_VERSION') )
			$not_installed .= 'Organic Beauty Utilities';
		$not_installed = apply_filters('organic_beauty_filter_importer_required_plugins', $not_installed, $list);
		if ($not_installed) {
			$this->error = '<b>'.esc_html__('Attention! For correct installation of the selected demo data, you must install and activate the following plugins: ', 'organic-beauty').'</b><br>'.($not_installed);
			return false;
		}
		return true;
	}
	...
	function exporter() {
		...
		// Export theme mods
		organic_beauty_fpc(organic_beauty_get_file_dir('core/core.importer/export/theme_mods.txt'), serialize($this->prepare_data(get_theme_mods())));

		// Export theme, templates and categories options and VC templates
		$rows = $wpdb->get_results( $wpdb->prepare( 
												"SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s",
												 organic_beauty_storage_get('options_prefix') . '_options%'
												 )
									);
		$options = array();
		if (is_array($rows) && count($rows) > 0) {
			foreach ($rows as $row) {
				$options[$row->option_name] = organic_beauty_unserialize($row->option_value);
			}
		}
		...
		organic_beauty_fpc(organic_beauty_get_file_dir('core/core.importer/export/theme_options.txt'), serialize($this->prepare_data($options)));

		// Export templates options
		$rows = $wpdb->get_results( $wpdb->prepare(
												"SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s",
												organic_beauty_storage_get('options_prefix').'_options_template_%'
												)
									);
		$options = array();
		if (is_array($rows) && count($rows) > 0) {
		...
		organic_beauty_fpc(organic_beauty_get_file_dir('core/core.importer/export/templates_options.txt'), serialize($this->prepare_data($options)));
		...
		organic_beauty_fpc(organic_beauty_get_file_dir('core/core.importer/export/widgets.txt'), serialize($this->prepare_data($options)));

		// Export posts
		organic_beauty_fpc(organic_beauty_get_file_dir('core/core.importer/export/posts.txt'), serialize(array(
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				$rows[$k]['user_pass']		= '';
				$rows[$k]['display_name']	= sprintf(esc_html__('User %d', 'organic-beauty'), $v['ID']);
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				else if ($v['meta_key'] == 'first_name')			$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'organic-beauty'), $v['user_id']);
				else if ($v['meta_key'] == 'last_name')				$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'organic-beauty'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_first_name')	$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'organic-beauty'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_last_name')		$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'organic-beauty'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_email')			$rows[$k]['meta_value'] = 'user'.$v['user_id'].'@user-mail.net';
			}
		}
		$users['usermeta'] = $rows;
		organic_beauty_fpc(organic_beauty_get_file_dir('core/core.importer/export/users.txt'), serialize($users));

		// Export Theme specific post types
		do_action('organic_beauty_action_importer_export', $this);
	...
	function export_dump($table) {
		...
		if ( count( $wpdb->get_results( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->prefix . trim($table) ), ARRAY_A ) ) == 1 ) {
			...
			if ($this->options['debug']) dfl(sprintf(__("Export %d rows from table '%s'. Used query: %s", 'organic-beauty'), count($rows), $table, $query));
		}
		return $rows;
	}
	...
	// Import dummy data
	//-----------------------------------------------------------------------------------
	//Handler of the add_action('wp_ajax_organic_beauty_importer_start_import',			array($this, 'importer'));
	//Handler of the add_action('wp_ajax_nopriv_organic_beauty_importer_start_import',	array($this, 'importer'));
	function importer() {

		if ($this->options['debug']) dfl(__('AJAX handler for importer', 'organic-beauty'));

		if ( !isset($_POST['importer_action']) || !wp_verify_nonce( organic_beauty_get_value_gp('ajax_nonce'), admin_url('admin-ajax.php') ) )
			die();
		...
		if ($this->options['debug']) dfl( sprintf(__('Dispatch action: %s', 'organic-beauty'), $this->action) );
		...
		$admin_tm = max(0, min(1800, (int) organic_beauty_get_theme_option('admin_dummy_timeout')));
		...
		// Import posts and users
			...
			if ($this->response['result'] >= 100 && $this->options['demo_set']=='full') {
				$this->import_users();
				do_action('organic_beauty_action_importer_after_import_posts', $this);
			}
		...
		// Import Theme specific posts
			...
			do_action('organic_beauty_action_importer_import', $this, $this->action);
		}
		...
		if ($this->options['debug']) dfl( sprintf(__("AJAX handler finished - send results to client: %s", 'organic-beauty'), json_encode($this->response)) );
	...
	function clear_tables() {
		...
		if ($this->options['demo_set']=='full') {
			if (organic_beauty_strpos($_POST['demo_parts'], 'posts')!==false) {
				if ($this->options['debug']) dfl( __('Clear tables', 'organic-beauty') );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->posts}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "posts".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->postmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "postmeta".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->comments}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "comments".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->commentmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "commentmeta".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->terms}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "terms".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_relationships}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "term_relationships".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_taxonomy}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "term_taxonomy".', 'organic-beauty' ) . ' ' . ($res->get_error_message()) );
			}
			do_action('organics_action_importer_clear_tables', $this, isset($_POST['clear_tables']) ? $_POST['clear_tables'] : '');
		}
	}
	...
	function import_users() {
		if ($this->options['debug']) 
			dfl(__('Import users', 'organic-beauty'));
		$this->response['start_from_id'] = 0;
		$this->import_dump('users', esc_html__('Users', 'organic-beauty'));
	}

	// Import posts, terms and comments
	function import_posts() {
		if ($this->options['debug']) 
			dfl(__('Import posts, terms and comments', 'organic-beauty'));
		...
		$this->import_dump('posts', esc_html__('Posts', 'organic-beauty'));
	}

	// Import media (uploads folder)
	function import_uploads() {
		if ($this->options['debug']) 
			dfl(__('Import media', 'organic-beauty'));
		...
		if ( ($txt = get_option('organic_beauty_import_uploads')) == '' ) {
			if ( ($txt = $this->get_file($this->options['files'][$this->options['demo_type']]['file_with_uploads'])) === false)
				return;
			else
				update_option('organic_beauty_import_uploads', $txt);
		}
		...
		foreach ($files as $file) {
			...
			if (organic_beauty_get_file_ext($file)=='zip') {
			...
			} else if (substr(organic_beauty_get_file_ext($file), 0, 1)==='0' && (int) organic_beauty_get_file_ext($file) > 0) {
				if ( ($txt = $this->get_file($file, round(max(0, $counter-1) / count($files) * 100))) === false)
					$need_exit = true;
				else {
					$zip = $this->uploads_dir.'/import_media.tmp';
					organic_beauty_fpc($zip, $txt, $file==$files[0] ? 0 : FILE_APPEND);
			...
			// Check time
			if ($this->options['debug']) 
				dfl(sprintf( __('File %s imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'organic-beauty'), $file, $result.'%', $this->max_time, time() - $this->start_time));
			// Unzip file
			if ($need_extract) {
				if (!empty($zip) && file_exists($zip)) {
					if ($this->options['debug']) 
						dfl(sprintf(__('Extract zip-file "%s"', 'organic-beauty'), $zip));
					if ( !unzip_file($zip, $this->uploads_dir) ) {
						$msg = sprintf(__('Error when unzip file "%s"', 'organic-beauty'), $zip);
					...
					$msg = sprintf(__('File "%s" not found', 'organic-beauty'), $zip);
					$this->response['error'] = $msg;
					if ($this->options['debug']) 
						dfl($msg);
				}
		...
		if ($result >= 100) update_option('organic_beauty_import_uploads', '');
	...
	function import_theme_mods() {
		if ($this->options['debug']) 
			dfl(__('Import Theme mods', 'organic-beauty'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'organic-beauty'), $this->options['files'][$this->options['demo_type']]['file_with_mods']));
		}
	}
	...
	function import_theme_options() {
		if ($this->options['debug']) 
			dfl(__('Import Theme Options', 'organic-beauty'));
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $k=>$v) {
				...
				update_option( $k, apply_filters('organic_beauty_filter_import_theme_options', $v, $k) );
			}
		} else {
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'organic-beauty'), $this->options['files'][$this->options['demo_type']]['file_with_options']));
		}
	}
	...
	function import_widgets() {
		if ($this->options['debug']) 
			dfl(__('Import Widgets', 'organic-beauty'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'organic-beauty'), $this->options['files'][$this->options['demo_type']]['file_with_widgets']));
		}
	}
	...
	function import_dump($slug, $title) {
		if ($this->options['debug']) 
			dfl(sprintf(__('Import dump file: "%s"', 'organic-beauty'), $this->options['files'][$this->options['demo_type']]['file_with_' . $slug]));
		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				if ($this->options['debug']) dfl(sprintf(__('Process table "%s"', 'organic-beauty'), $table));
				...
				if ($this->options['demo_set']=='part' && $table=='posts' && $this->response['start_from_id'] > 0) {
					$this->part_replace = get_option('organic_beauty_importer_part_replace', array());
					$this->part_image = get_option('organic_beauty_importer_part_image', array());
				}
				if (is_array($rows) && ($posts_all=count($rows)) > 0) {
					...
					foreach ($rows as $row) {
						...
						if (!apply_filters('organic_beauty_filter_importer_import_row', $this->options['demo_set']=='full', $table, $row, $this->options['demo_parts'])) continue;
						// Replace demo URL to current site URL
						$row = organic_beauty_replace_site_url($row, $this->options['files'][$this->options['demo_type']]['domain_demo']);
						$f = '';
						$v = '';
						...
						if ($this->options['debug']) {
							dfl( sprintf( __('Record (ID=%s) is imported. Progress: %s. Time: %s sec. from %s sec.', 'organic-beauty'),
											!empty($row['ID']) 
				...
				if ($this->options['demo_set']=='part' && $table=='posts') {
					update_option('organic_beauty_importer_part_replace', $result < 100 ? $this->part_replace : array());
					update_option('organic_beauty_importer_part_image', $result < 100 ? $this->part_image : array());
				}
				if ($this->options['debug']) dfl(sprintf(__('Imported %s. Elapsed time %s sec. of %s sec.', 'organic-beauty'), $result.'%', time() - $this->start_time, $this->max_time));
				if ($break) break;
			}
		} else {
			if ($this->options['debug']) 
				dfl(sprintf(__('Error unserialize data from the file %s', 'organic-beauty'), $this->options['files']['demo_type']['file_with_' . $slug]));
		}
		$this->response['result'] = $result;
	}
	
	// Check if the row will be imported
	// Handler of the add_filter('organic_beauty_filter_importer_import_row', array($this, 'import_check_row'), 9, 4);
	...
	function import_prepare_no_image() {
		$no_image_title = esc_html__('No-Image placeholder', 'organic-beauty');
		$no_image_post = get_page_by_title($no_image_title, OBJECT, 'attachment');
		if ( empty($no_image_post->ID) ) {
			if ( ($img = organic_beauty_get_file_dir("images/no-image.jpg")) != '') {
				// Copy to the 'uploads' folder
				$this->part_image = wp_upload_bits( 'no-image.jpg', 0, organic_beauty_fgc($img));
	...
	function get_list_pages_callback() {
		if ( !wp_verify_nonce( organic_beauty_get_value_gp('ajax_nonce'), admin_url('admin-ajax.php') ) )
			die();

		$this->prepare_vars();

		$response = array(
			'error' => empty($_POST['demo_type']) ? esc_html__('Incorrect parameters', 'organic-beauty') : '',
		);
	...
	function replace_uploads($str) {
		return organic_beauty_replace_uploads_url($str);
	}
	...
	function prepare_data($str) {
		$need_ser = false;
		if (is_string($str) && organic_beauty_substr($str, 0, 2)=='a:') {
		...
		if (is_array($str) && count($str) > 0) {
			...
			if ($this->options['files'][$this->options['demo_type']]['domain_dev']!=$this->options['files'][$this->options['demo_type']]['domain_demo'])
				$str = str_replace(
							organic_beauty_get_domain_from_url($this->options['files'][$this->options['demo_type']]['domain_dev']),
							organic_beauty_get_domain_from_url($this->options['files'][$this->options['demo_type']]['domain_demo']),
							$str);
	...
	function download_file($fname, $result=0) {
		...
		if (substr($fname, 0, 5)=='http:' || substr($fname, 0, 6)=='https:') {
			...
			if (is_string($response)) {
				...
				if ($this->options['debug']) 
					dfl(sprintf(__('Download file %s successful', 'organic-beauty'), $fname));
			} else {
				if ($attempt < 3) {
					$this->response['attempt'] = $attempt;
					$this->response['result'] = $result;
					if ($this->options['debug']) {
						$error_log = sprintf(__("Attempt %d. Download file '%s' failed.", 'organic-beauty'), $attempt, $fname);
						dfl($error_log);
					}
				} else {
					unset($this->response['attempt']);
					$this->response['error'] = sprintf(__("Error download file '%s'.", 'organic-beauty'), $fname)
												. " \n" . __("Please, try again!", 'organic-beauty');
					if ($this->options['debug']) 
	...
	function get_file($fname, $result=0) {
		$attempt = !empty($_POST['attempt']) ? (int) $_POST['attempt']+1 : 1;
		$fname = trailingslashit($this->options['demo_url']) . trim($this->options['demo_type']) . '/' . trim($fname);
		$txt = organic_beauty_fgc($fname);
		if (empty($txt)) {
			if ($attempt < 3) {
				$this->response['attempt'] = $attempt;
				$this->response['result'] = $result;
				if ($this->options['debug']) {
					$error_log = sprintf(__("Attempt %d. Load data from the file '%s' failed. ", 'organic-beauty'), $attempt, $fname);
					dfl($error_log);
				}
			} else {
				unset($this->response['attempt']);
				$this->response['error'] = sprintf(__("Error load data from the file '%s'.", 'organic-beauty'), $fname)
											. " \n" . __("Please, try again!", 'organic-beauty');
				if ($this->options['debug']) 
					dfl($this->response['error']);
			}
			...
			unset($this->response['attempt']);
			if ($this->options['debug']) 
				dfl(sprintf(__('Load data from the file %s successful', 'organic-beauty'), $fname));
		}
		return $txt;
	}
}
