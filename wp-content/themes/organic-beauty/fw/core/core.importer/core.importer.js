// Organic Beauty Importer script
...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_demo_type input[type="radio"]').on('change', function() {
		...
		var data = {
			ajax_nonce: ORGANIC_BEAUTY_STORAGE['ajax_nonce'],
			action: 'organic_beauty_importer_get_list_pages',
			demo_type: type
		};
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		ORGANIC_BEAUTY_STORAGE['importer_error_messages'] = '';
		ORGANIC_BEAUTY_STORAGE['importer_ignore_errors'] = true;
		organic_beauty_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function organic_beauty_importer_do_action(steps, idx) {
	"use strict";
	...
	var data = {
		ajax_nonce: ORGANIC_BEAUTY_STORAGE['ajax_nonce'],
		action: 'organic_beauty_importer_start_import',
	...
	jQuery.post(ORGANIC_BEAUTY_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || ORGANIC_BEAUTY_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				ORGANIC_BEAUTY_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				organic_beauty_importer_do_action(steps, idx);
			} else {
				if (ORGANIC_BEAUTY_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + ORGANIC_BEAUTY_STORAGE['importer_error_msg'] + '</h4>' + ORGANIC_BEAUTY_STORAGE['importer_error_messages']);
				} else {
