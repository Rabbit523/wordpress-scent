<?php
/**
 * Organic Beauty Framework: templates and thumbs management
 *
...
// Theme init
if (!function_exists('organic_beauty_templates_theme_setup')) {
	add_action( 'organic_beauty_action_before_init_theme', 'organic_beauty_templates_theme_setup' );
	function organic_beauty_templates_theme_setup() {

		// Add custom thumb sizes into media manager
		add_filter( 'image_size_names_choose', 'organic_beauty_show_thumb_sizes');
	}
}
...
if (!function_exists('organic_beauty_add_template')) {
	function organic_beauty_add_template($tpl) {
		...
		organic_beauty_storage_set_array('registered_templates', $tpl['layout'], $tpl);
		if (!empty($tpl['thumb_title']) || !empty($tpl['thumb_slug']))
			organic_beauty_add_thumb_sizes( $tpl );
		else 
			$tpl['thumb_title'] = '';
	}
}
...
if (!function_exists('organic_beauty_get_template_name')) {
	function organic_beauty_get_template_name($layout_name) {
		$tpl = organic_beauty_storage_get_array('registered_templates', $layout_name);
		return !empty($tpl['template']) ? $tpl['template'] : $tpl['layout'];
	}
}

// Return true, if template required content
if (!function_exists('organic_beauty_get_template_property')) {
	function organic_beauty_get_template_property($layout_name, $what) {
		$tpl = organic_beauty_storage_get_array('registered_templates', $layout_name);
...
if (!function_exists('organic_beauty_get_template_function_name')) {
	function organic_beauty_get_template_function_name($layout_name) {
		$tpl = organic_beauty_storage_get_array('registered_templates', $layout_name);
		return 'organic_beauty_template_'.str_replace(array('-', '.'), '_', $tpl['template']).'_output';
	}
}

// Set template arguments
if (!function_exists('organic_beauty_template_set_args')) {
	function organic_beauty_template_set_args($tpl, $args) {
		organic_beauty_storage_push_array('call_args', $tpl, $args);
	}
}


// Get template arguments
if (!function_exists('organic_beauty_template_get_args')) {
	function organic_beauty_template_get_args($tpl) {
		return organic_beauty_storage_pop_array('call_args', $tpl, array());
	}
}


// Look for last template arguments (without removing it from storage)
if (!function_exists('organic_beauty_template_last_args')) {
	function organic_beauty_template_last_args($tpl) {
		$args = organic_beauty_storage_get_array('call_args', $tpl, array());
...
if (!function_exists('organic_beauty_add_thumb_sizes')) {
	function organic_beauty_add_thumb_sizes($sizes) {
		static $mult = 0;
		if ($mult == 0) $mult = min(4, max(1, organic_beauty_get_theme_option("retina_ready")));
		if (!isset($sizes['h_crop']))		$sizes['h_crop'] =  isset($sizes['h']) ? $sizes['h'] : null;
		//if (empty($sizes['mode']))		$sizes['mode'] = 'blog';
		if (empty($sizes['thumb_title']))	$sizes['thumb_title'] = organic_beauty_strtoproper(!empty($sizes['layout']) ? $sizes['layout'] : $sizes['thumb_slug']);
		$thumb_slug = !empty($sizes['thumb_slug']) ? $sizes['thumb_slug'] : organic_beauty_get_slug($sizes['thumb_title']);
		if (empty($sizes['layout']))		$sizes['layout'] = $thumb_slug;
		if (organic_beauty_storage_get_array('thumb_sizes', $thumb_slug)=='') {
			organic_beauty_storage_set_array('thumb_sizes', $thumb_slug, $sizes);
			// Register WP thumb size
			if (organic_beauty_get_theme_setting('add_image_size') && (!isset($sizes['add_image_size']) || $sizes['add_image_size'])) {
				// Image thumb and retina version
				add_image_size( 'organic_beauty-'.$thumb_slug, $sizes['w'], $sizes['h'], $sizes['h']!=null );
				if ($mult > 1)
					add_image_size( 'organic_beauty-'.$thumb_slug.'-@retina', $sizes['w'] ? $sizes['w']*$mult : $sizes['w'], $sizes['h'] ? $sizes['h']*$mult : $sizes['h'], $sizes['h']!=null );
				// Cropped image thumb and retina version
				if ($sizes['h']!=$sizes['h_crop']) {
					add_image_size( 'organic_beauty-'.$thumb_slug.'_crop', $sizes['w'], $sizes['h_crop'], true );
					if ($mult > 1) 
						add_image_size( 'organic_beauty-'.$thumb_slug.'_crop-@retina', $sizes['w'] ? $sizes['w']*$mult : $sizes['w'], $sizes['h_crop'] ? $sizes['h_crop']*$mult : $sizes['h_crop'], true );
				}
			}
		}
	}
...
if (!function_exists('organic_beauty_get_thumb_sizes')) {
	function organic_beauty_get_thumb_sizes($opt) {
		$opt = array_merge(array(
			'layout' => 'excerpt',
			'thumb_slug' => ''
		), $opt);
		$tpl = organic_beauty_storage_get_array('registered_templates', $opt['layout']);
		$thumb_slug = !empty($opt['thumb_slug']) 
						? $opt['thumb_slug'] 
						: (empty($tpl['thumb_slug']) 
							? (empty($tpl['thumb_title']) 
								? '' 
								: organic_beauty_get_slug($tpl['thumb_title'])
								) 
							: organic_beauty_get_slug($tpl['thumb_slug'])
							);
		$thumb_size = organic_beauty_storage_get_array('thumb_sizes', $thumb_slug);
		return !empty($thumb_size) ? $thumb_size : array('w'=>null, 'h'=>null, 'h_crop'=>null);
	}
}

// Show custom thumb sizes into media manager sizes list
if (!function_exists('organic_beauty_show_thumb_sizes')) {
	function organic_beauty_show_thumb_sizes( $sizes ) {
		$thumb_sizes = organic_beauty_storage_get('thumb_sizes');
