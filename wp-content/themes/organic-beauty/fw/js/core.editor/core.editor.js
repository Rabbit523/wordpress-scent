jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(ORGANIC_BEAUTY_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == ORGANIC_BEAUTY_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(ORGANIC_BEAUTY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				organic_beauty_message_success('', ORGANIC_BEAUTY_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(ORGANIC_BEAUTY_STORAGE['strings']['editor_caption_close']);
			} else {
				organic_beauty_message_warning(rez.error, ORGANIC_BEAUTY_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		organic_beauty_message_confirm(ORGANIC_BEAUTY_STORAGE['strings']['editor_delete_post'], ORGANIC_BEAUTY_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(ORGANIC_BEAUTY_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					organic_beauty_message_success('', ORGANIC_BEAUTY_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = ORGANIC_BEAUTY_STORAGE['site_url'];
						}, 1000);
				} else {
					organic_beauty_message_warning(rez.error, ORGANIC_BEAUTY_STORAGE['strings']['editor_delete_error']);
				}
			});
