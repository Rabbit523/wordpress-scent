...
class WC_nets extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title = "Nets ";
        $this->method_description = "Accept payments through Netaxept Nets";
        ...
        $this->easy_payment         = ( isset($this->settings['easy_payment']) && $this->settings['easy_payment']=="yes" ) ? true : false;
        $this->recurring_payment    = ( isset($this->settings['recurring_payment']) && $this->settings['recurring_payment']=="yes" ) ? true : false;

        $this->test_mode 			= (isset($this->settings['test_mode']) && $this->settings['test_mode'] == "yes") ? true : false;
        $this->debug_mode 			= (isset($this->settings['debug_mode']) && $this->settings['debug_mode'] == "yes") ? true : false;
        ...
        if (!$this->test_mode) { // Use live fields
            ...
            $this->merchant_id 		= (isset($this->settings['test_merchant_id'])) ? $this->settings['test_merchant_id'] : '';
            $this->token 			= (isset($this->settings['test_token'])) ? $this->settings['test_token'] : '';
        }
        ...
        else $this->enabled = (isset($this->settings['enabled'])) ? 'yes' : 'no';
    ...
    public function init_form_fields() {
        $this->form_fields = [
            ...
            'easy_payment' => [
                'title' => __('Easy payment', 'woocommerce-gateway-nets'),
                'type' => 'checkbox',
                'label' => __('Do you have an easy payment agreement?', 'woocommerce-gateway-nets'),
                'default' => 'no'
            ],
            'recurring_payment' => [
                'title' => __('Recurring payment', 'woocommerce-gateway-nets'),
                'type' => 'checkbox',
                'label' => __('Do you have an recurring payment agreement?', 'woocommerce-gateway-nets'),
                'default' => 'no'
            ],
            ...
            'test_mode' => array(
                ...
                'description' => __('<b>Requires Test Keys</b>: Fill in the test keys below.', 'woocommerce-gateway-nets'),
                'default' => 'yes'
            ),
            'test_merchant_id' => array(
                'title' => __('Nets Test Merchant ID ', 'woocommerce-gateway-nets'),
                'type' => 'text',
                'description' => __('Please enter your nets Test Merchant ID; this is needed in order to use nets\' test mode.', 'woocommerce-gateway-nets'),
                'default' => ''
            ),
            'test_token' => array(
                'title' => __('Nets Test Token', 'woocommerce-gateway-nets'),
                'type' => 'text',
                'description' => __('Please enter your Nets Test Token.', 'woocommerce-gateway-nets'),
                'default' => ''
            ),
            ...
            'debug_mode' => array(
                ...
                'type' => 'checkbox',
                'label' => __('Enable debug mode logging (<a href="/wp-admin/admin.php?page=wc-status&tab=logs">Show logs</a>)', 'woocommerce-gateway-nets'),
                'default' => 'no'
            ),
    ...
    public function process_order_with_nets($order, $transactionId) {
    ...
    */
        ...
        foreach ($order->get_items() as $item) {
            if ( $item->is_type( 'line_item' ) && ( $product = $item->get_product() ) ) {
                ...
                if ( $item->get_total_tax() == 0 ? false : true ) {
                    ...
                    $goods_item["isVatIncluded"] = $item->get_total_tax() == 0 ? false : true;
                }
    ...
    public function info( $string ) {
        if($this->debug_mode) $this->log->info($string,["source"=>"nets"]);
    }
    public function debug( $string ) {
        if($this->debug_mode) $this->log->debug($string,["source"=>"nets"]);
    }
    ...
    public function payment_complete_hook($order_id) {
        $nets = new WC_nets();
        $nets->debug('payment_complete_hook() - called for: '.$order_id);

        if ($nets->capture_type == 'autocapture' ) return; // Jump out if autocapture through nets is activated
        ...
        if ($transactionId != '') {
            ...
            // Run query to obtain info on transaction
            try {
                $query_data = $nets->api->send_request(new  NetsQueryRequest([
                    "merchantId" => $nets->merchant_id,
                    "token" => $nets->token,
                    "transactionId" => $transactionId,
                ]));

                $nets->debug("Query request response: " . var_export($query_data, true));
            }
            catch (Exception $e) {
                $msg = __('Unable to query nets.','woocommerce-gateway-nets');


                $nets->debug("Query request error: " . $e);
            ...
            if ( in_array($method,$nets->special_invoice_methods) ) {
                //$order->add_order_note(__('Nets: Invoice payment ', 'woocommerce-gateway-nets'));
                return true;
            }

            try {
                $process_data = $nets->api->send_request(new  NetsProcessRequest([
                    "merchantId" => $nets->merchant_id,
                    "token" => $nets->token,
                    "operation" => "CAPTURE",
                    "transactionId" => $transactionId,
                ...
                $nets->debug("Process(CAPTURE) request response: " . var_export($process_data, true));
                ...
                $nets->debug("Process(CAPTURE) request error: " . $e);
    ...
    public function nets_callback($data) {
        ...
        if ($transactionId && $responseCode == 'OK') { // Terminal phase was successfull
            ...
            // First we check whether an payment method that doesn't support AUTH/CAPTURE or SALE commands
            // If so we go directly to the success part
            // If not we check whether it is already authorized or not and then act acordingly
    ...
    // Show icons in chackout title
    function nets_gateway_icon( $icon, $id ) {
        if ($id === 'nets') { // Filter Nets Gateway
            $nets 	= new WC_nets();

            if($nets->title_logo){
                return $icon . $nets->get_icon_string();
            }
            return $icon;
        }
    }
